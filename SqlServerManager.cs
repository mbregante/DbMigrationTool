...
namespace DbMigrationTool
{
  ...
  internal static class DataReaderExtensions
  {
    ...
    public static string GetString(this SqlDataReader dr, string colName)
    {
      ...
      throw new Exception(string.Format("Column {0} does not exists.", colName));
    }

    public static string GetStringNullable(this SqlDataReader dr, string colName)
    {
      if (dr.HasColumn(colName))
      {
        if (dr.IsNotNull(colName))
          return dr.GetString(dr.GetOrdinal(colName));
      }
    ...
    public static int? GetInt32Nullable(this SqlDataReader dr, string colName)
    {
      if (dr.HasColumn(colName))
      {
        if (dr.IsNotNull(colName))
    ...
    public static bool? GetBooleanNullable(this SqlDataReader dr, string colName)
    {
      if (dr.HasColumn(colName))
      {
        if (dr.IsNotNull(colName))
    ...
    public static DateTime? GetDateTimeNullable(this SqlDataReader dr, string colName)
    {
      if (dr.HasColumn(colName))
      {
        if (dr.IsNotNull(colName))
  ...
  internal class SqlServerManager
  {
    ...
    internal bool CanCreateDatabase(string databaseName)
    {
      try
      {
        if (m_connStr.Contains("database.windows.net"))
        {
          return false;
        }
    ...
    internal void GetDatabaseInfo(out DatabaseInfo databaseInfo, out DatabaseServerInfo serverInfo, bool generateScript = false)
    {
      ...
      using (SqlConnection connection = new SqlConnection(m_connStr))
      {
        ...
        bool isAzure = m_connStr.Contains("database.windows.net");
